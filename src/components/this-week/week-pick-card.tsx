...
import type { WeekPick } from "@/lib/week-pick";

export function WeekPickCard({ pick }: { pick: WeekPick }) {
  const { listing, note } = pick;
    ...
    <article className="min-w-0">
      <Card className="h-full gap-0 bg-card py-0">
        <div className="relative">
          ...
          />
          <MallPickBadge className="absolute top-3 left-3 bg-card/90 text-foreground" />
        </div>
